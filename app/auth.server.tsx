...
export async function authenticate(request: Request, headers = new Headers()): Promise<{ accessToken: string; headers?: Headers }> {
  try {
    ...
    if (error instanceof AuthorizationError) {
      ...
      headers.append("Set-Cookie", await commitSession(session, { maxAge: 7 * 24 * 60 * 60 * 1 }));
...
export async function createUserSession({ request, access_token, refresh_token, expires_at, user, redirectTo }: IcreateUserSession) {
  const session = await getSession(request.headers.get("Cookie"));
